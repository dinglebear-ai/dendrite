...
> **For agentic workers:** Follow the create-artifacts plan contract and the target project’s agent instructions. Implement only approved work task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.
...
**Tech Stack:** [Key technologies and libraries, e.g. the verified language, framework, runtime, and test runner.]
...
- [Version floor, e.g. the project’s supported runtime versions.]
- [Dependency limit, e.g. no new runtime dependencies.]
- [Naming or copy rule, e.g. all user-facing strings use the project’s localization layer.]
- [Platform requirement, e.g. supported desktop and mobile environments.]
...
| File | Responsibility |
| --- | --- |
| `src/[area]/[thing].ext` | [What this module owns, in one line.] |
| `src/[area]/[thing]/[part].ext` | [What this part owns.] |
| `tests/[area]/[thing]_test.ext` | [What this proves.] |
...
**Files:**
- Create: `src/[area]/[thing].ext`
- Modify: `src/[area]/[caller].ext:120-138`
- Test: `tests/[area]/[thing]_test.ext`
...
```text
# tests/[area]/[thing]_test.ext
defmodule Project.[Area].[Thing]Test do
  use ExUnit.Case, async: true

  alias Project.[Area].[Thing]
...
Run: `[verified-test-command] tests/[area]/[thing]_test.ext:8`
Expected: FAIL — `** (UndefinedFunctionError) function Project.[Area].[Thing].[function]/1 is undefined`

- [ ] **Step 3: Write the minimal implementation**

```text
# src/[area]/[thing].ex
defmodule Project.[Area].[Thing] do
...
Run: `[verified-test-command] tests/[area]/[thing]_test.ext`
Expected: PASS — `1 test, 0 failures`

- [ ] **Step 5: Commit**

```bash
git add tests/[area]/[thing]_test.ext src/[area]/[thing].ex
...
**Interfaces:**
- Consumes: `Project.[Area].[Thing].[function]/1` → `{:ok, [type]} | {:error, [reason]}`
...
4. **Rendering conventions** — these strings are load-bearing, because
   ...
   - Every fence carries a language tag (```text, ```bash) or its code
     renders uncoloured.
